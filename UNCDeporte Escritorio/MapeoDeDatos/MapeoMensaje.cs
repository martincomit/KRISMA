...
namespace MapeoDeDatos
{
	public class MapeoMensaje
	{
        ...
        public DataSet ConsultarMensajes()
        {
            string sql = "";
            SqlConnection cx = new SqlConnection();
            DataSet ds = new DataSet();

            sql = @"select mt.id_mensaje, mt.fecha, mt.mensaje,  t.id_torneo, t.nombre, t.cantidad_participantes
                from torneos t, mensajesXTorneo mt
                    where t.id_torneo = mt.id_torneo";

            StringConexion c = new StringConexion();
            cx.ConnectionString = c.getCadena();
            SqlDataAdapter da = new SqlDataAdapter(sql, cx);
            da.Fill(ds, "MensajesXTorneo");
            return ds;
        ...
        public void EliminarMensaje(int id_mensaje)
        {
            try
            {
                string sql = "";
                SqlConnection cx = new SqlConnection();
                sql = "Delete from MensajesXTorneo where id_mensaje=" + id_mensaje; //ojo q aca elimino por torneo, y en web por deporte en autogestion 
